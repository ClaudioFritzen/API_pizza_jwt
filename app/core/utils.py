...
def verificar_token(token: str = Depends(oauth2_scheme), session: SessionType = Depends(get_db)):

    try:
        dic_info = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        usuario_id = int(dic_info.get("sub"))
    except JWTError as erro:
        raise HTTPException(status_code=401, detail=f"Acesso Negado, verifique a validade do token: {erro}")
